...
#define TAIL_ANGLE_DRIVE      3 /* バランス走行時の角度[度] 3*/
#define P_GAIN             2.5F /* 完全停止用モータ制御比例係数 2.5*/
#define PWM_ABS_MAX         60 /* 完全停止用モータ制御PWM絶対最大値 60*/
/* sample_c4マクロ */
#define DEVICE_NAME       "ET322"  /* Bluetooth通信用デバイス名 */
...
TASK(TaskMain)
{
	...
	signed int fangle = 30; // 傾倒時のオフセット角
	...
	while(1) {
		...
		case 11: /*** 通常走行中 ***/
			...
			//if (tripmeter() > 19900) pattern = 100;/*19.9mでストップ*/
			...
			//if (tripmeter() > 16500) speed = 75; else speed = 75; //16500

			if (sonar < 30 && tripmeter()> 0 && course == OUT) // ルックアップゲート検知ならpattern=20 16000
			{
				pattern = 20;
				counter = 0;
				break;
			...
			if(tripmeter() > 400 && check_Seesaw()>2){
				counter = 0;
				pattern = 40;
			}
...
//			line_follow3(speed, black2, white2);
		...
		case 40:
			line_follow(0, turn, gyro_sensor);
			if (counter > 10 ){
				counter = 0;
				pattern = 41;
			}
